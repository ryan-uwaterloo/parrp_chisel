...
class SinkA(params: InclusiveCacheParameters) extends Module
{
  val io = IO(new Bundle {
    ...
    val pb_beat = new PutBufferAEntry(params) // this is to say that SinkA connects to SourceD?
  })
  ...
  params.ccover(a.valid && buf_block, "SINKA_BUF_STALL", "No space in putbuffer for beat")//FYI beat is a data packet in TL spec
  ...
  io.req.bits.prio   := VecInit(1.U(3.W).asBools)//A channel is bottom prio
