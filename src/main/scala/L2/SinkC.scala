...
class SinkC(params: InclusiveCacheParameters) extends Module
{
  ...
  if (params.firstLevel) {
    ...
    when (c.valid && c.ready){
      ...
      printf(cf"@ clk_cycle ${clk_cycle}: New Sink C Request! opcode: AccessAckData, addr: 0x${c.bits.address}%x, source: 0x${c.bits.source}%x, data: 0x${c.bits.data}%x\n")
      ...
      printf(cf"@ clk_cycle ${clk_cycle}: New Sink C Request! opcode: ProbeAckData, addr: 0x${c.bits.address}%x, source: 0x${c.bits.source}%x, data: 0x${c.bits.data}%x\n")
      ...
      printf(cf"@ clk_cycle ${clk_cycle}: New Sink C Request! opcode: ReleaseData, addr: 0x${c.bits.address}%x, source: 0x${c.bits.source}%x, data: 0x${c.bits.data}%x\n")
    }
    }
